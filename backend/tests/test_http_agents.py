...
def test_context_encoded_traversal_blocked(client, tmp_path):
    ...
    # (partial match: {agent_id}/{filename}=".." hit, trailing /evil.md not
    # consumed, PUT registered on the route). The property under test: never
    # 200, and nothing written outside context/.
    resp = client.put(
