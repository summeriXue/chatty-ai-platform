"""WeCom integration — Message processing service.

Routes inbound WeCom text messages to Chatty and returns
...
def _resolve_agent() -> dict | None:
    """Resolve the Chatty agent assigned to WeCom."""
    creds = get_credentials("wecom")
    agent_id = creds.get("agent_id", "")

    if not agent_id:
        logger.warning(
            "WeCom integration has no agent assigned"
        )
        return None
    ...
    if not agent:
        logger.warning(
            "WeCom assigned agent not found: %s",
            agent_id,
        )
        return None
...
async def process_message(
    ...
    """Process a WeCom private text message through Chatty."""
    ...
    integration_tool_defs, integration_executors = (
        _load_integration_tools()
    )
    ...
    # Get or create the persistent WeCom conversation mapping.
    wecom_conversation = state.get_or_create_conversation(
        sender_id=sender_id,
        agent_id=agent["id"],
    )

    chatty_conversation_id = wecom_conversation.get(
        "chatty_conversation_id"
    )

    # A WeCom user + Chatty agent should keep using the same
    ...
    if not chatty_conversation_id and chat_service:
        try:
            ...
            state.set_chatty_conversation_id(
                wecom_conversation["id"],
                chatty_conversation_id,
            )

            logger.info(
                "Created Chatty conversation for WeCom sender=%s agent=%s",
                sender_id,
                agent["id"],
            )
            ...
            logger.warning(
                "Failed to create WeCom conversation: %s",
                exc,
            )
