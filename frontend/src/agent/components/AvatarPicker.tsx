  ...
  const handleGenerate = async () => {
    ...
    try {
      ...
      if (err instanceof DOMException && err.name === 'AbortError') {
        setError(t('avatarPicker.errors.generationTimedOut'));
      } else {
        setError(
          err instanceof Error
            ? err.message
            : t('avatarPicker.errors.generationFailed')
        );
      }
    } finally {
      clearTimeout(timer);
  ...
  const handleSelect = async (index: number) => {
    ...
    try {
      ...
      setError(
        err instanceof Error
          ? err.message
          : t('avatarPicker.errors.saveFailed')
      );
      setSelecting(false);
    }
  };
  ...
  const handleUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      if (!res.ok) {
        ...
        throw new Error(body.detail || t('avatarPicker.errors.uploadFailed'));
      }
      ...
      setError(
        err instanceof Error
          ? err.message
          : t('avatarPicker.errors.uploadFailed')
      );
    } finally {
      setUploading(false);
  ...
  return (
    <div className="max-w-lg mx-auto px-6 py-16 text-center">
      ...
      <h2 className="text-xl font-bold text-white mb-2">
        {t('avatarPicker.title', { name: agentName })}
      </h2>

      <p className="text-ch-ink-mute text-sm mb-6">
        {openaiAvailable
          ? t('avatarPicker.descriptionGenerate', { name: agentName })
          : t('avatarPicker.descriptionUpload', { name: agentName })}
      </p>
      ...
      {!started && (
        <div className="flex flex-wrap gap-3 justify-center">
          {openaiAvailable && (
            ...
            >
              {t('avatarPicker.generateAvatars')}
            </button>
          )}
      ...
      {(generating || uploading) && (
        <div className="flex flex-col items-center gap-3 py-8">
          ...
          <p className="text-sm text-ch-ink-mute">
            {uploading
              ? t('avatarPicker.uploading')
              : t('avatarPicker.generating')}
          </p>
        </div>
      )}
        ...
        <div className="bg-red-900/30 text-red-400 text-sm rounded-lg p-4 mb-4">
          ...
          <div className="flex gap-3 justify-center">
            ...
            <button
              ...
              className="underline font-medium"
            >
              {t('avatarPicker.uploadYourOwnLower')}
            </button>
          </div>
        </div>
      )}
          ...
          <div className="grid grid-cols-3 gap-4">
            {urls.map((url, i) => (
              ...
              >
                <img
                  src={url}
                  alt={t('avatarPicker.avatarOption', { number: i + 1 })}
